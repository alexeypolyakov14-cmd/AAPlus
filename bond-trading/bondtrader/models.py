...
_SERIES_PREFIXES = ("ПБО", "БО", "БП", "ПБ", "ЗО", "СУБ", "ПК", "GOLD", "SILV", "SUB", "SB", "BO", "П", "Б", "Р", "Т")
_SERIES_TOKEN_RE = re.compile(r"^(?:(?:%s)-?)*[0-9]" % "|".join(_SERIES_PREFIXES), re.I)


def _is_series_token(tok: str) -> bool:
    """Маркер серии/выпуска, а не слово имени: цепочка известных префиксов (с дефисами или без) перед цифрой —
    «БО-П15», «ПБО-08», «ЗО28-1-Р», «СУБ-Т1-Р1», «Sb42R», «GOLD01», «001Р-06»; «ТГК-14» и «А101» — имена."""
    return bool(_SERIES_TOKEN_RE.match(tok)) or (len(tok) >= 2 and tok.upper() in _SERIES_PREFIXES)   # «Т Плюс»: одна буква — слово
...
def issuer_key_of_full(full_name: str, words: int = 2) -> str:
    ...
    for t in s.split():
        t = t.strip(",.;:")
        # маркеры сектора MOEX: «i» — инновации, «s» — устойчивое развитие (iКаршеринг Руссия, sГТЛК) — не часть имени
        if len(t) >= 3 and t[0] in ("i", "s") and re.match(r"[А-Яа-яЁё]", t[1]):
            t = t[1:]
        # серия, приклеенная к имени без пробела: «Аэрофьюэлз-002Р-04», «Аэрофьюэлз002Р-06», «АЛИУМ01Р1» → имя;
        # «ТГК-14» / «АБЗ-1» (дефис + номер без хвоста) — часть имени
        t = re.sub(r"(?<=[A-Za-zА-Яа-яЁё]{3})(?:-?[0-9]{3,}|[0-9]{2,}[A-Za-zА-Яа-яЁё]|[0-9][A-Za-zА-Яа-яЁё][0-9]).*$", "", t)
